...
class ResConfigSettings(models.TransientModel):
    ...
    def get_values(self):
        ...
        penalty_option = ICPSudo.get_param(
            'ae_loan_management.penalty_option')
        invoice_type = ICPSudo.get_param(
            'ae_loan_management.invoice_type')
        charge_option = ICPSudo.get_param(
            'ae_loan_management.charge_option')
        charge = float(ICPSudo.get_param(
            'ae_loan_management.charge'))
        of_days = int(ICPSudo.get_param(
            'ae_loan_management.of_days'))
        down_payment = bool(ICPSudo.get_param(
            'ae_loan_management.down_payment'))
    ...
    def set_values(self):
        ...
        ICPSudo = self.env['ir.config_parameter'].sudo()
        ICPSudo.set_param(
            'ae_loan_management.down_payment', self.down_payment)
        charge_option = self.charge_option
        if self.penalty_option == 'interest':
            charge_option = 'percentage'
        ICPSudo.set_param(
            'ae_loan_management.penalty_option', self.penalty_option)
        ICPSudo.set_param(
            'ae_loan_management.invoice_type', self.invoice_type)
        ICPSudo.set_param(
            'ae_loan_management.charge_option', charge_option)
        ICPSudo.set_param('ae_loan_management.charge', self.charge)
        ICPSudo.set_param(
            'ae_loan_management.of_days', self.of_days or 2)
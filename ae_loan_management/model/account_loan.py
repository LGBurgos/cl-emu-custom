...
class AccountLoan(models.Model):
    ...
    processing_agent_fee_amt = fields.Float(string="Processing with Agent Fee",compute="total_processing_agent")

    @api.depends('ln_processing_fee','ln_agent_fee','loan_amount')
    def total_processing_agent(self):
        for rec in self:
            rec.processing_agent_fee_amt = 0
            if rec.prcessing_fee_amt and rec.ln_agent_fee:
                rec.processing_agent_fee_amt = rec.prcessing_fee_amt + (rec.loan_amount * rec.ln_agent_fee / 100)
            elif rec.prcessing_fee_amt:
                rec.processing_agent_fee_amt = rec.prcessing_fee_amt
            elif rec.ln_agent_fee:
                rec.processing_agent_fee_amt = (rec.loan_amount * rec.ln_agent_fee / 100)
    ...
    def setting_down_payment_show(self):
        for loan in self:
            loan.down_payment_show = self.env['ir.config_parameter'].sudo().get_param('ae_loan_management.down_payment', False)
    ...
    def compute_amount_ln_inv_total(self):
        for loan in self:
            loan.amount_due = 0
            loan_move_type = self.env['ir.config_parameter'].sudo().get_param('ae_loan_management.invoice_type')
    ...
    def cal_princi_inter(self):
        ...
        interest_prod_id = self.env.company and self.env.company.sudo().loan_interest_prod_id
        for loan in self.search([]):
            principle_domain = [('move_id.loan_id', '=', loan.id),('account_id.account_type', '=', 'asset_receivable'),('journal_id.type','=','general')]
    ...
    def view_insurance_balance(self):
        ...
        if self.insurance_product_id:
            action = self.env.ref('ae_loan_management.loan_invoice_line_action')
            result = action.read()[0]
    ...
    def view_principal_balance(self):
        ...
        domain = [('move_id.loan_id', '=', self.id),('account_id.account_type', '=', 'asset_receivable'),('journal_id.type', '=', 'general')]
        invoice_lines = self.env['account.move.line'].search(domain).ids
    ...
    def view_interest_balance(self):
        ...
        action = self.env.ref('ae_loan_management.action_loan_line_smt')
        result = action.read()[0]
    ...
    def view_loan_transactions(self):
        ...
        action = self.env.ref('ae_loan_management.action_loan_transaction')
        result = action.read()[0]
    ...
    def view_payments(self):
        ...
        """
        self.ensure_one()
        action = self.env.ref('ae_loan_management.view_payments')
        result = action.read()[0]
        return result
    ...
    def create_processing_fees_entry(self,fee_product,agent_fee_product,disbursement_journal_id,loan_process_payble_id):
        if not fee_product.property_account_expense_id:
            raise UserError(_("Please Configure Expense Account Into Process Fee Product"))
        if not agent_fee_product.property_account_expense_id:
            raise UserError(_("Please Configure Expense Account Into Agent Fee Product"))
        inv_obj = self.env['account.move']
        today = datetime.today().date()

        if self.processing_agent_fee_amt:
            processing_move = inv_obj.create({
            'journal_id': disbursement_journal_id,
            'loan_id': self.id or False,
            'ref': 'Processing/Agent Fees Entry',
            'date': today,
            'company_id': self.env.company.id,
             })
            lines = []
            if self.prcessing_fee_amt:
                lines.append({
                'account_id': fee_product.property_account_expense_id.id,
                'debit': 0,
                'credit': self.prcessing_fee_amt,
                'date': today,
                'ref': 'Processing Fees Expense',
                })
                lines.append({
                'account_id': loan_process_payble_id,
                'credit': 0,
                'debit': self.prcessing_fee_amt,
                'date': today,
                'ref': 'Processing Fees Payable',
                })
            if self.ln_agent_fee:
                lines.append({
                'account_id': agent_fee_product.property_account_expense_id.id,
                'credit': self.processing_agent_fee_amt - self.prcessing_fee_amt,
                'debit':  0,
                'date': today,
                'ref': 'Agent Fees Payable',
                })
                lines.append({
                'account_id': loan_process_payble_id,
                'debit': self.processing_agent_fee_amt - self.prcessing_fee_amt,
                'credit':  0,
                'date': today,
                'ref': 'Agent Fees Expense',
                })
         
            processing_move.line_ids = [(0, 0, val) for val in lines]
            processing_move.action_post()
    ...
    def post_loan(self):
        ...
        if not self.line_ids:
           raise UserError('Please Compute Loan Items first after that you can post the loan!')
            ...
            loan_receivable_acc_id = self.partner_id and self.partner_id.property_account_receivable_id and self.partner_id.property_account_receivable_id.id or False
            if not loan_receivable_acc_id:
                raise UserError(_("Please Configure 'Loan Customer Account Receivable' from Loans -> Configurations -> Settings!"))
            ...
            loan_process_payble_id = self.env.company.sudo().loan_process_payble_id and \
                                  self.env.company.sudo().loan_process_payble_id.id or False
            if not loan_process_payble_id:
                raise UserError(_("Please Configure Processing/Agent Fee Payable Account Into Current Company!"))
            ...
            fees_deduct_amount = loan_amt - self.processing_agent_fee_amt
            ...
            if self.ln_processing_fee:
                lines = [{
                    'account_id': loan_receivable_acc_id,
                    'partner_id': partner and partner.id or False,
                    'debit': loan_amt,
                    'credit': 0,
                    'date': today,
            ...
            #============Jornal entry for the processing Fees===============#
            self.create_processing_fees_entry(fee_product,agent_fee_product,disbursement_journal_id,loan_process_payble_id)
    ...
    def due_date_selection(self):
        ...
        :return:
        """
        view_id = self.env.ref('ae_loan_management.move_due_date_of_loan_wizard', False)
        return {
    ...
    def send_loan_email(self):
        ...
        for loan in loan_rec:
            mail_template = loan.env.ref('ae_loan_management.email_template_loan_details')
    ...
    def action_loan_detail_send(self):
        ...
        '''
        for loan in self:
            template_id = self.env['ir.model.data']._xmlid_to_res_id('ae_loan_management.email_template_loan_details', raise_if_not_found=False)
